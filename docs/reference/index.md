...
| `boringssl-inspector` | BoringSSL Inspector | full | BoringSSL/TLS 检查域，支持 TLS 流量分析和证书检查。 |
| `browser` | Browser | full | 浏览器控制与 DOM 交互主域，也是大多数工作流的入口。 |
...
| `core` | Core | full | 核心静态/半静态分析域，覆盖脚本采集、反混淆、语义理解、webpack/source map 与加密识别。 |
| `cross-domain` | Cross-Domain | full | 跨域关联域，将多个域的分析结果进行交叉关联，支持自动化工作流编排与证据图桥接。 |
| `debugger` | Debugger | full | 基于 CDP 的断点、单步、调用栈、watch 与调试会话管理域。 |
| `encoding` | Encoding | full | 二进制格式检测、编码转换、熵分析与 protobuf 原始解码。 |
...
| `graphql` | GraphQL | full | GraphQL 发现、提取、重放与 introspection 能力。 |
...
| `maintenance` | Maintenance | full | 运维与维护域，覆盖缓存、token 预算、环境诊断、产物清理与扩展管理。 |
| `memory` | Memory | full | 面向原生内存扫描、指针链分析、结构体推断与断点观测的内存分析域。 |
| `mojo-ipc` | Mojo IPC | full | Mojo IPC 监控域，用于 Chromium 内部进程间通信分析。 |
| `network` | Network | full | 请求捕获、响应体读取、HAR 导出、请求重放与性能追踪。 |
...
| `skia-capture` | Skia Capture | full | Skia 渲染引擎捕获域，用于 UI 渲染分析和可视化。 |
| `sourcemap` | SourceMap | full | SourceMap 发现、抓取、解析与源码树重建。 |
| `streaming` | Streaming | full | WebSocket 与 SSE 监控域。 |
...
| `v8-inspector` | V8 Inspector | full | V8 检查器域，提供堆快照分析、CPU 分析和内存检查。 |
| `wasm` | WASM | full | WebAssembly dump、反汇编、反编译、优化与离线执行域。 |
| `workflow` | Workflow | full | 复合工作流与脚本库域，是 built-in 高层编排入口。 |
