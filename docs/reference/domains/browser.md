...
| `browser_get_metrics` | 原子原语：通过 PerformanceMonitor 采集页面性能指标 — Web Vitals (FCP, LCP, CLS, TTFB)、DOM 时序 (domContentLoaded, loadComplete)、引擎级计数器 (scriptDuration, layoutDuration, recalcStyleDuration) 与 JS 堆大小 (usedJSHeapSize / totalJSHeapSize / jsHeapSizeLimit)。可选包含原始 performance timeline 条目。取代旧的网络域 performance_get_metrics（仍作为向后兼容别名可用）。 |
| `browser_trace_start` | 原子原语：在活跃页面通过 page.tracing.start() 开始一次 Chrome 性能 trace。与 browser_trace_stop 配对使用以将 trace 保存到磁盘。有明确假设时建议传入合理的 categories 列表（如 ["devtools.timeline","v8.execute","blink.user_timing"]）；默认集合覆盖大多数性能分析场景。 |
| `browser_trace_stop` | 原子原语：停止由 browser_trace_start 启动的 Chrome 性能 trace，并持久化到 artifacts/traces/（或自定义路径）。返回事件数、文件大小与 Chrome DevTools 查看提示。若从未开始或已停止 trace 则明确报错。 |
| `browser_cpu_profile_start` | 原子原语：在活跃页面开始 CDP CPU 采样分析（Profiler.start）。与 browser_cpu_profile_stop 配对使用以保存 .cpuprofile。需要高分辨率画像时可将 samplingInterval 设为 30-100 µs（默认 1000 µs / 1 ms）。 |
| `browser_cpu_profile_stop` | 原子原语：停止 CDP CPU 采样分析，按采样次数排列热点函数，并将原始 profile 持久化到 artifacts/profiles/（或自定义路径）。热点函数列表由 samples 数组推导 — 新版 Chrome 的 profile 不再填充 hitCount。若从未开始采样则明确报错。 |
