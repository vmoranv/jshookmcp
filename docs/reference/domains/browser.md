...
- `get_detailed_data` — 根据上一次工具返回的 detailId 获取完整数据。
- `browser_attach` — 连接到一个正在运行的浏览器。
- `browser_list_tabs` — 列出浏览器中所有打开的标签页。
- `browser_list_cdp_targets` — 列出当前已连接浏览器目标可见的所有 CDP 目标。
- `browser_select_tab` — 切换到指定的标签页。
...
| `page_navigate` | 跳转到指定网址。 |
| `page_reload` | 重新加载当前页面。 |
| `page_back` | 后退到上一个页面。 |
| `page_forward` | 前进到下一个页面。 |
| `page_click` | 点击页面上的元素。 |
| `page_type` | 在输入框中输入文字。 |
| `page_select` | 在下拉菜单中选择选项。 |
| `page_hover` | 将鼠标移到元素上方。 |
| `page_scroll` | 滚动页面。 |
| `page_wait_for_selector` | 等待某个元素出现。 |
| `page_evaluate` | 在页面中执行 JavaScript 代码。 |
| `page_screenshot` | 截取页面或元素的截图。 |
...
| `page_press_key` | 模拟按下键盘按键，如回车、ESC、方向键。 |
| `captcha_detect` | 检测页面上是否有验证码。 |
| `captcha_wait` | 等待用户手动完成验证码。 |
| `captcha_config` | 配置验证码检测和自动处理。 |
| `stealth_inject` | 注入反检测脚本，降低被网站识别为自动化的概率。 |
| `stealth_set_user_agent` | 设置更真实的浏览器标识信息。 |
| `stealth_configure_jitter` | 配置 CDP 命令时序抖动，在每个 CDP send() 调用间注入随机延迟以防止基于时序的自动化检测。 |
| `stealth_generate_fingerprint` | 生成逼真的浏览器指纹（屏幕、WebGL、navigator、字体等）。 |
| `stealth_verify` | 检测当前浏览器是否容易被网站识别为自动化工具，返回 0-100 分。 |
...
| `framework_state_extract` | 提取页面中 React/Vue/Svelte 等框架的组件数据，用于调试或逆向分析。 |
| `indexeddb_dump` | 导出浏览器本地数据库中存储的数据。 |
| `js_heap_search` | 在浏览器内存中搜索指定字符串（如 token、密钥）。 |
