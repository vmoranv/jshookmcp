...
- `mojo-ipc`, `v8_bytecode_extract`, `wasm`, CAPTCHA solving, and some platform helpers still have explicit fallback/simulation or partial-output paths even when capability probes succeed. `mojo_ipc_capabilities` now separates backend probe availability from still-unavailable live Mojo capture. `captcha_solver_capabilities` now separates manual mode, 2captcha-compatible external mode, unsupported providers, and current-page hook availability. `wasm_capabilities` now separates current-page browser capture from external WABT/Binaryen/runtime availability. `v8_jit_inspect` now returns native optimization status on the default Chrome launch path, but `v8_bytecode_extract` still does not expose raw Ignition bytecode.
