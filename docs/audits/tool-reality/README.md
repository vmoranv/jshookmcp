...
- Focused runtime probes on this machine confirmed real payloads for Frida attach/module/script execution, network response bodies, WebSocket frames, SSE events, trace body/chunk capture, and V8 heap snapshot capture/analyze. Mojo monitoring still degraded to simulation mode with a seeded default interface catalog, and `v8_heap_stats` remained partial on this machine.
...
- `mojo-ipc`, `v8_heap_stats`, `wasm`, CAPTCHA solving, and some platform helpers still have explicit fallback/simulation or partial-output paths even when capability probes succeed.
