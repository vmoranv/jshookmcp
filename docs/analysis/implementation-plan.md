...
- [ ] Add unit tests for Tool Router
- [ ] Add integration tests with search_tools
...
- [ ] Upgrade webcrack if newer version available
- [x] Add pnpm overrides for isolated-vm if needed (isolated-vm@6.1.2)
- [ ] Move webcrack to optionalDependencies (with feature gating)
- [ ] Add doctor command to check native dependencies
- [ ] Update documentation for Node version requirements
...
**Status**: 🚧 Starting Phase 1-2

**Target Files**:
- `src/server/ToolSearch.ts` (819 lines) → Split into:
  - `src/server/search/ToolSearchEngine.ts`
  - `src/server/search/BM25Scorer.ts`
  - `src/server/search/IntentBoost.ts`
  - `src/server/search/AffinityGraph.ts`
- `src/server/MCPServer.search.ts` (572 lines) → Split into:
  - `src/server/search/handleSearchTools.ts`
  - `src/server/search/handleActivateTools.ts`
  - `src/server/search/handleActivateDomain.ts`
...
### Week 3
- In Progress: Documentation optimization
  - [x] Create troubleshooting guide (docs/guide/troubleshooting.md)
  - [ ] Simplify tool tables using collapsible sections
  - [ ] Audit and consolidate EN/ZH documentation
  - [ ] Add common search issue solutions
- ⏳ Optimize dynamic boost strategy
- ⏳ Begin file size refactoring
