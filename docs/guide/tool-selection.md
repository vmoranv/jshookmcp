...
`jshookmcp` 实现了声明式的动态工具路由架构（249+ Tools / 17 Domains），采用命名空间与按需激活隔离。严禁在调度方硬编码工具签名预期，一切关联依赖和预热必须通过路由总线动态解析。
...
| `full`            | 全域静态预载 (17 Domains)                                                                  | 全量挂载 249+ 工具，抹平按需加载延迟，适于重型静态分析与全栈审计   | 零       |
...
- **持久化上下文隔离**：`web_api_capture_session` 会将请求序列自动序列化为 HAR 并落盘，断开连接后可通过归档文件做 Context 重建，无需维持 Headless 长连接。
