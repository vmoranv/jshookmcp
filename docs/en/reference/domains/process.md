...
| `inject_dll` | Inject a DLL into a target process using CreateRemoteThread + LoadLibraryA (Windows) or gdb/lldb (Linux/macOS). Enabled by default; set ENABLE_INJECTION_TOOLS=false to disable. Requires administrator privileges. |
| `inject_shellcode` | Inject and execute shellcode in a target process. Accepts hex or base64. Enabled by default; set ENABLE_INJECTION_TOOLS=false to disable. |
