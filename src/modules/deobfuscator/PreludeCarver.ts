...
export interface PreludeFunction {
  ...
  category: 'decoder' | 'rotator' | 'wrapper' | 'vm-bootstrap' | 'integrity' | 'anti-debug' | 'string-table' | 'opcode-derivation';
...
export function detectPrelude(code: string): PreludeFunction[] {
  ...
  try {
      ...
      FunctionDeclaration(path) {
        ...
        for (const pattern of DECODER_PATTERNS) {
          if (pattern.test(source)) {
            preludeFunctions.push({ name: fnName, category: 'decoder', confidence: 0.85, snippet, nodeType, evaluated: false });
            return;
          }
        }
        ...
        for (const pattern of ROTATOR_PATTERNS) {
          if (pattern.test(source)) {
            preludeFunctions.push({ name: fnName, category: 'rotator', confidence: 0.8, snippet, nodeType, evaluated: false });
            return;
          }
        }
        ...
        for (const pattern of WRAPPER_PATTERNS) {
          if (pattern.test(source)) {
            preludeFunctions.push({ name: fnName, category: 'wrapper', confidence: 0.75, snippet, nodeType, evaluated: false });
            return;
          }
        }
        ...
        for (const pattern of VM_BOOTSTRAP_PATTERNS) {
          if (pattern.test(source)) {
            preludeFunctions.push({ name: fnName, category: 'vm-bootstrap', confidence: 0.9, snippet, nodeType, evaluated: false });
            return;
          }
        }
        ...
        for (const pattern of INTEGRITY_PATTERNS) {
          if (pattern.test(source)) {
            preludeFunctions.push({ name: fnName, category: 'integrity', confidence: 0.7, snippet, nodeType, evaluated: false });
            return;
          }
        }
        ...
        for (const pattern of ANTI_DEBUG_PATTERNS) {
          if (pattern.test(source)) {
            preludeFunctions.push({ name: fnName, category: 'anti-debug', confidence: 0.75, snippet, nodeType, evaluated: false });
            return;
          }
        }
        ...
        for (const pattern of OPCODE_DERIVATION_PATTERNS) {
          if (pattern.test(source)) {
            preludeFunctions.push({ name: fnName, category: 'opcode-derivation', confidence: 0.8, snippet, nodeType, evaluated: false });
            return;
          }
        }
      },
        ...
        if (t.isArrayExpression(init) && init.elements.length > 15) {
          ...
          if (hasStrings || hasHex) {
            for (const pattern of STRING_TABLE_PATTERNS) {
              ...
              if (pattern.test(source)) {
                const snippet = source.slice(0, 200);
                preludeFunctions.push({ name: varName, category: 'string-table', confidence: 0.85, snippet, nodeType: 'VariableDeclarator', evaluated: false });
                return;
              }
            }
          }
    ...
    logger.warn(`PreludeCarver: AST parse failed, falling back to regex detection: ${e instanceof Error ? e.message : String(e)}`);
    // Regex fallback
    for (const pattern of [...DECODER_PATTERNS, ...ROTATOR_PATTERNS, ...WRAPPER_PATTERNS]) {
      const match = code.match(pattern);
      if (match) {
        preludeFunctions.push({ name: '', category: 'decoder', confidence: 0.6, snippet: match[0].slice(0, 200), nodeType: 'regex-match', evaluated: false });
      }
    }
  }
  ...
  for (const p of jsDefPatterns) {
    preludeFunctions.push({ name: p.pattern, category: 'integrity', confidence: p.confidence, snippet: p.pattern, nodeType: 'JSDefender', evaluated: false });
  }
  ...
  for (let i = 0; i < evaluated.length; i++) {
    ...
    if (pf.category === 'string-table' && pf.name) {
      try {
        ...
        const arrayMatch = code.match(new RegExp(`(?:var|let|const)\\s+${pf.name}\\s*=\\s*\\[([\\s\\S]*?)\\]`));
        if (arrayMatch?.[1]) {
          const evalCode = `
            try {
    ...
    if (pf.category === 'decoder' && pf.name) {
      try {
        ...
        while ((match = decoderCallPattern.exec(code)) !== null && resolutions.length < 100) {
          ...
          `;
          const result = await sandbox.execute({ code: evalCode, timeoutMs: Math.min(timeoutMs, 3000) });
          if (result.ok && typeof result.output === 'string' && result.output !== 'ERROR') {
...
export function carvePrelude(code: string, preludeFunctions: PreludeFunction[]): PreludeCarverResult {
  ...
  for (const line of lines) {
    const isPrelude = line.trim().length > 0 && preludeNames.size > 0 && (
      Array.from(preludeNames).some((name) => {
        if (!name) return false;
        // Check if line defines or references this prelude function
        const lineDefRegexp = new RegExp(`(?:function\\s+${name}|var\\s+${name}|let\\s+${name}|const\\s+${name}|${name}\\s*=)`);
...
export async function carvePreludeFromCode(
  ...
  logger.info(`PreludeCarver: detected ${preludeFunctions.length} prelude functions: ${preludeFunctions.map((pf) => `${pf.name}(${pf.category})`).join(', ')}`);
  ...
  logger.info(`PreludeCarver: complete in ${Date.now() - startTime}ms, ${result.replaced} calls replaced`);
