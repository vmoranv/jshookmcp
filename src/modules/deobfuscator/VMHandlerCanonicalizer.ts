...
const CATEGORY_PATTERNS: Array<{
  ...
  },
  {
    ...
    patterns: [
      (n) => /\b(push|unshift|shift)\s*\(/.test(generate(n).code) && /\b=\s*[\w$]+\s*[\+\-](\d+|[\w$]+)/.test(generate(n).code),
  ...
  },
  {
    category: 'memory',
    patterns: [
      (n) => /\[\s*['"][\w$]+['"]\s*\]/.test(generate(n).code) && !/\bpush\b|\bpop\b/.test(generate(n).code),
      (n) => /\bvar\b[\s\S]*=\s*\w+\[/.test(generate(n).code) && /\b=\s*\w+\[/.test(generate(n).code),
    ],
    stackPush: 1,
    stackPop: 1,
  },
  {
    category: 'string-op',
    patterns: [
      (n) => /\b(charAt|substring|slice|split|indexOf|replace|toLowerCase|toUpperCase|trim)\b/.test(generate(n).code),
...
const TOOL_SIGNATURES: Array<{
  ...
  },
  {
    ...
    patterns: [
      /console\[('|`|")/,
      /debugger;/,
      /function\s+\w+\s*\(\s*\)\s*\{\s*debugger\s*;/,
    ],
  },
];
 ...
 */
export function compareGenomes(a: OpcodeGenome, b: OpcodeGenome): {
  similarity: number;
  sameVM: boolean;
  ...
  const toolMatch = (a.toolIdentifier && b.toolIdentifier && a.toolIdentifier === b.toolIdentifier) ? 0.3 : 0;
      ...
      for (const stmt of body.body) {
        if (t.isSwitchStatement(stmt)) {
          ...
          if (caseCount >= 5) {
            ...
            const hasIntegrityChecks = /debugger/.test(source) ||
              /self\s*\|\|/.test(source) ||
              /console\[/.test(source);
...
function extractHandlers(ds: VMDispatchStructure): VMHandler[] {
  ...
  for (const switchCase of ds.node.cases) {
    ...
    handlers.push({
      ...
      handlerNode: switchCase.consequent.length > 0 ? switchCase.consequent[0]! as t.Statement : null,
      canonicalSource,
...
function classifyHandler(handler: VMHandler): void {
  ...
  for (const categoryPattern of CATEGORY_PATTERNS) {
      ...
      if (handler.handlerNode) {
        try {
          ...
          if (categoryPattern.category === 'stack-op') {
            ...
            regexPatterns.push(/\b(charAt|substring|slice|split|indexOf|replace|toLowerCase|toUpperCase|trim)\s*\(/);
          }
    ...
    if (categoryPattern.category === 'stack-op') {
      ...
      if (/STR\d+/.test(handlerSource) && /\b(charAt|substring|slice|split|indexOf|replace)\b/.test(handlerSource)) matchCount++;
    } else if (categoryPattern.category === 'environment') {
      if (/\b(window|document|navigator|location)\b/.test(handlerSource)) matchCount++;
    }

    const confidence = categoryPattern.patterns.length > 0
...
function emptyGenome(): OpcodeGenome {
  return {
    ...
    handlerCount: 0,
    categoryHistogram: {
      'stack-op': 0, 'arithmetic': 0, 'comparison': 0, 'logic': 0,
      'control-flow': 0, 'memory': 0, 'string-op': 0, 'type-coercion': 0,
      'environment': 0, 'crypto': 0, 'unknown': 0,
    },
    handlers: [],
