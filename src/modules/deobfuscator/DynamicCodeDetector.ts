...
export interface DynamicCodeDetection {
  type: 'eval' | 'newFunction' | 'import' | 'setTimeout' | 'setInterval' | 'setImmediate' | 'Function' | 'vm' | 'wasm' | 'reflect' | 'angular' | 'react';
  location: string;
  code: string;
}
...
export function detectDynamicCodePatterns(code: string): DynamicCodeDetection[] {
  ...
  const patterns: Array<{ type: DynamicCodeDetection['type']; regex: RegExp }> = [
    ...
    { type: 'setTimeout', regex: /setTimeout\s*\(\s*(?:function|.+\.toString\(\))\s*,\s*\d+\s*\)/s },
    { type: 'setInterval', regex: /setInterval\s*\(\s*(?:function|.+\.toString\(\))\s*,\s*\d+\s*\)/s },
...
export async function executeDynamicCode(
  ...
  for (const detection of detections) {
    try {
      if (detection.type === 'eval' || detection.type === 'newFunction' || detection.type === 'Function') {
        ...
        if (sandboxResult.ok && typeof sandboxResult.output === 'string') {
          ...
          warnings.push(`${detection.type}: captured generated code (${sandboxResult.output.length} chars)`);
        }
      }
    } catch (error) {
      warnings.push(`${detection.type}: failed to execute: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
export function inlineDynamicCode(code: string, options?: InlineDynamicCodeOptions): { code: string; inlined: number } {
  let inlined = 0;
  const opts = {
    stripEval: true,
    stripImport: true,
        ...
        if (
          ...
          if (t.isFunctionExpression(firstArg) || t.isArrowFunctionExpression(firstArg)) {
            if (t.isBlockStatement(firstArg.body)) {
              const stmts = firstArg.body.body.filter(
