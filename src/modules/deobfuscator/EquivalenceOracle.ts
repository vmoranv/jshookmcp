      ...
      VariableDeclarator(path) {
        if (t.isFunctionExpression(path.node.init) || t.isArrowFunctionExpression(path.node.init)) {
          if (t.isIdentifier(path.node.id)) {
            const paramCount = (path.node.init as t.FunctionExpression | t.ArrowFunctionExpression).params.length;
            sigs.add(`fn:${path.node.id.name}:${paramCount}`);
          }
        }
      },
...
export async function checkEquivalence(
  ...
  checks.push({
    ...
    details: syntaxCheck.valid ? 'Deobfuscated code parses successfully' : `Parse error: ${syntaxCheck.error}`,
    severity: 'critical',
  });
  if (!syntaxCheck.valid) {
    ...
    return {
      ...
      shouldRollback: true,
      warnings,
      delta: { literalsAdded: 0, literalsRemoved: 0, functionsAdded: 0, functionsRemoved: 0, exportsChanged: false, controlFlowChanged: false },
    };
  }
  ...
  const literalsRemoved = [...meaningfulOrigLiterals].filter((l) => !meaningfulDeobfLiterals.has(l)).length;
  const literalsAdded = [...meaningfulDeobfLiterals].filter((l) => !meaningfulOrigLiterals.has(l)).length;

  const literalPreservationRate = meaningfulOrigLiterals.size > 0
