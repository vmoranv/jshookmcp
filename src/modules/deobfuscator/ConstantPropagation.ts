...
export function advancedConstantPropagation(code: string): ConstantPropagationResult {
  ...
  try {
    ...
    warnings.push(`Constant propagation failed: ${error instanceof Error ? error.message : String(error)}`);
...
function isPureExpression(node: t.Node): boolean {
  ...
  if (t.isCallExpression(node)) {
    ...
    if (t.isIdentifier(callee)) {
      return ['String', 'Number', 'Boolean', 'Array', 'Object', 'Math', 'JSON'].includes(callee.name);
    }
    return false;
  }
