...
export interface WASMBoundary {
  ...
  /** Type of boundary */
  type: 'instantiate' | 'compile' | 'inline-bytes' | 'base64-encoded' | 'fetch-load' | 'constructor';
...
export function harvestWASM(code: string, options?: WASMHarvesterOptions): WASMExtractionResult {
  ...
  try {
    ...
    for (const extraction of extractionResults) {
      ...
      if (extraction.bytes && extraction.bytes.length > 0) {
        ...
        if (!header.hasValidMagic) {
          warnings.push(`WASM module ${moduleCount} has invalid magic bytes (may be encrypted or WASMicer-obfuscated)`);
        }
    ...
    if (!moduleCount && boundaries.length > 0) {
      ...
      warnings.push(`${boundaries.length} WASM boundaries detected but no inline WASM bytes found (may be externally loaded)`);
    }
...
function extractWASMBytes(code: string, opts: Required<WASMHarvesterOptions>): WASMByteExtraction[] {
  ...
  for (const extraction of inlineArrays) {
    if (extraction.bytes && extraction.bytes.length > 8) {
      // Validate WASM magic
      if (extraction.bytes[0] === 0x00 && extraction.bytes[1] === 0x61 &&
          extraction.bytes[2] === 0x73 && extraction.bytes[3] === 0x6d) {
  ...
  for (const result of results) {
    if (result.bytes && result.bytes.length > opts.maxModuleSize) {
      logger.warn(`WASMHarvester: truncating ${result.bytes.length} bytes to ${opts.maxModuleSize}`);
...
function parseMemorySection(bytes: Uint8Array, start: number, _size: number): { minPages: number; maxPages?: number; shared: boolean } {
  let offset = start;
  const { value: count } = readLEB128(bytes, offset);
...
function parseExportNames(bytes: Uint8Array, start: number, _size: number, _count: number): string[] {
  const names: string[] = [];
  let offset = start;
...
function traceJSWASMInterface(code: string, _bytes: Uint8Array | null, header: WASMModuleHeader): WASMJSInterface {
    ...
    traverse(ast, {
      ...
      CallExpression(path) {
        ...
        if (
          ...
          if ((method === 'instantiate' || method === 'instantiateStreaming') && path.node.arguments.length >= 2) {
...
function extractStringsFromWASM(bytes: Uint8Array, header: WASMModuleHeader): { offset: number; value: string }[] {
  ...
  for (let i = startOffset; i < endOffset; i++) {
    ...
    if (isPrintable) {
      ...
      if (currentStringBytes.length >= 4) {
        try {
          const value = new TextDecoder('utf-8', { fatal: false }).decode(new Uint8Array(currentStringBytes));
...
function looksLikeWASMByteString(content: string): boolean {
  ...
  const hasMagic = /0x00/.test(content) && /0x61/.test(content) && /0x73/.test(content) && /0x6d/.test(content);
