...
export function getWASMMixedSchemeSummary(code: string): { threat: 'low' | 'medium' | 'high'; details: string } {
...
export function detectWASMBytecodePayloads(code: string): Array<{ type: 'base64' | 'hex' | 'string'; payload: string; line: number }> {
...
export function extractWASMMetadata(wasmBytes: Uint8Array): {
  ...
  const version = hasMagic
    ? ((wasmBytes[4]! << 0) | (wasmBytes[5]! << 8) | (wasmBytes[6]! << 16) | (wasmBytes[7]! << 24)) >>> 0
    : null;

  return { hasMagic, version, exports, imports, rawSize };
}

export async function decodeWASMBytecodeEmbedding(
  code: string,
): Promise<WASMBytecodeDecodeResult> {
  const warnings: string[] = [];
  ...
  if (firstPayload.type === 'base64') {
    try {
      ...
      if (metadata.hasMagic) {
        warnings.push(`Decoded WASM binary: magic ${metadata.hasMagic ? 'valid' : 'invalid'}, version ${metadata.version ?? 'unknown'}`);
