...
function synthesizeShadow(
  ...
  const evalCaptures = captures.filter((c) => c.category === 'eval-source' || c.category === 'function-source');
  ...
  if (wasmCaps.length > 0) {
    ...
    lines.push('  throw new Error("WASM execution not reconstructed — see reconstruction report");');
    lines.push('}');
    lines.push('');
  }
  ...
  if (timerCaps.length > 0) {
    ...
    lines.push('  // Decoded content available in reconstruction report');
    lines.push('  throw new Error("Deferred execution not reconstructed — see reconstruction report");');
    lines.push('}');
    lines.push('');
  }
  ...
  if (cryptoCaps.length > 0) {
    ...
    lines.push('  throw new Error("Crypto operation not reconstructed — see reconstruction report");');
    lines.push('}');
    lines.push('');
  }
...
export async function reconstructBehavior(
  ...
  const method = captures.length > 0 && evaluatedPreludes.length > 0
    ? 'hybrid' as const
    : captures.length > 0
  ...
  logger.info(`BehavioralReconstructor: complete in ${Date.now() - startTime}ms, method=${method}, confidence=${confidence.toFixed(2)}`);
