...
export const aiHookTools: Tool[] = [
  tool('ai_hook_generate')
    ...
    .prop('behavior', {
      ...
      properties: {
        ...
        modifyReturn: { type: 'boolean', description: 'Allow return value modification', default: false },
        blockExecution: { type: 'boolean', description: 'Block the original function from executing', default: false },
    ...
    .prop('condition', {
      type: 'object',
      properties: {
        argFilter: { type: 'string', description: "JS expression to filter by args (e.g., \"args[0].includes('password')\")" },
  ...
  tool('ai_hook_inject')
    ...
    .enum('method', ['evaluateOnNewDocument', 'evaluate'], 'Injection method', { default: 'evaluate' })
...
export const hookPresetTools: Tool[] = [
  tool('hook_preset')
    ...
    .boolean('captureStack', 'Include call stack in captured data (has performance impact)', { default: false })
