...
export const advancedTools: Tool[] = [
  ...
  tool('network_disable')
    .desc('Disable network request monitoring')
    .destructive()
    .build(),
  ...
  tool('network_get_requests')
    ...
    .number('limit', 'Maximum number of results per page (default: 100, max: 1000)', { default: 100 })
    .number('offset', 'Skip first N results for pagination (default: 0). Use page.nextOffset from previous response.', { default: 0 })
    .boolean('autoEnable', 'Auto-enable network monitoring when currently disabled', { default: true })
  ...
  tool('network_get_response_body')
    ...
    .boolean('returnSummary', 'Return only size and preview instead of full body', { default: false })
    ...
    .boolean('autoEnable', 'Auto-enable network monitoring when currently disabled', { default: false })
    .boolean('enableExceptions', 'When autoEnable=true, also enable uncaught exception monitoring', { default: true })
    .required('requestId')
    .build(),
  ...
  tool('performance_take_heap_snapshot')
    .desc('Take a V8 heap memory snapshot')
    .build(),
