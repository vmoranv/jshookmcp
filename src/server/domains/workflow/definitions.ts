...
export const workflowToolDefinitions: Tool[] = [
  tool('web_api_capture_session')
    ...
    .number('waitAfterActionsMs', 'Milliseconds to wait after all actions before collecting', { default: 1500 })
    .required('url')
    .openWorld()
    .build(),
  ...
  tool('js_bundle_search')
    ...
    .boolean('cacheBundle', 'Cache the bundle for 5 minutes to avoid re-downloads', { default: true })
  ...
  tool('page_script_run')
    ...
    .prop('params', { type: 'object', additionalProperties: true, description: 'Optional parameters injected as __params__ (must be JSON-serializable)' })
    .required('name')
    .openWorld()
    .build(),
  ...
  tool('register_account_flow')
    ...
    .string('submitSelector', 'CSS selector for the submit button', { default: "button[type='submit']" })
  ...
  tool('batch_register')
    ...
    .string('submitSelector', 'Default submit button selector for all accounts', { default: "button[type='submit']" })
  ...
  tool('run_extension_workflow')
    ...
    .prop('config', { type: 'object', additionalProperties: true, description: 'Optional config overrides read through ctx.getConfig(path, fallback)' })
    .prop('nodeInputOverrides', { type: 'object', additionalProperties: { type: 'object', additionalProperties: true }, description: 'Optional shallow input overrides keyed by workflow node id' })
