...
describe('network http2-raw frame builder', () => {
  it('builds a minimal DATA frame with payload', () => {
  ...
  it('builds a DATA frame with text payload', () => {
  ...
  it('builds a SETTINGS ack frame with no payload', () => {
  ...
  it('throws when SETTINGS ack includes settings', () => {
    expect(() =>
      buildHttp2Frame({
        frameType: 'SETTINGS',
        ack: true,
  ...
  it('builds a PING frame with opaque data', () => {
    const result = buildHttp2Frame({
      frameType: 'PING',
  ...
  it('builds a PING frame with zero-filled opaque data when omitted', () => {
    const result = buildHttp2Frame({ frameType: 'PING' });
    expect(result.payloadBytes).toBe(8);
    expect(result.payloadHex).toBe('0000000000000000');
  });

  it('builds a PING ack frame', () => {
    const result = buildHttp2Frame({
      frameType: 'PING',
      ack: true,
  ...
  it('throws when WINDOW_UPDATE omits windowSizeIncrement', () => {
    expect(() => buildHttp2Frame({ frameType: 'WINDOW_UPDATE' })).toThrow(
      'windowSizeIncrement is required',
    );
  });

  it('builds a RST_STREAM frame with error code', () => {
    const result = buildHttp2Frame({
  ...
  it('builds a RST_STREAM frame with default error code 0', () => {
    const result = buildHttp2Frame({ frameType: 'RST_STREAM', streamId: 1 });
    expect(result.payloadHex).toBe('00000000');
  });

  it('builds a GOAWAY frame with lastStreamId and errorCode', () => {
    const result = buildHttp2Frame({
      frameType: 'GOAWAY',
      lastStreamId: 7,
      errorCode: 11,
  ...
  it('builds a GOAWAY frame with debug data text', () => {
    const result = buildHttp2Frame({
      frameType: 'GOAWAY',
      lastStreamId: 0,
      errorCode: 0,
  ...
  it('builds a GOAWAY frame with ascii debug data', () => {
    const result = buildHttp2Frame({
      frameType: 'GOAWAY',
      debugDataText: 'ok',
  ...
  it('builds a RAW frame with explicit type code', () => {
    const result = buildHttp2Frame({
      frameType: 'RAW',
      frameTypeCode: 0xff,
      payloadHex: 'ab',
  ...
  it('throws when RAW frame omits frameTypeCode', () => {
  ...
  it('throws when settings value exceeds uint32', () => {
    expect(() =>
      buildHttp2Frame({
        frameType: 'SETTINGS',
  ...
  it('throws for mutually exclusive payloadHex and payloadText', () => {
    expect(() =>
      buildHttp2Frame({
        frameType: 'DATA',
        streamId: 1,
  ...
  it('uses ascii encoding for DATA payloadText', () => {
    const result = buildHttp2Frame({
      frameType: 'DATA',
      streamId: 1,
      payloadText: 'A',
  ...
  it('produces correct 9-byte header for a small frame', () => {
    const result = buildHttp2Frame({
      frameType: 'DATA',
      streamId: 1,
