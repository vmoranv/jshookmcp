...
describe('config.schema.json', () => {
  ...
  it('loads and parses config schema', async () => {
    const { readFile } = await import('node:fs/promises');
    const { resolve } = await import('node:path');
    const content = await readFile(resolve('config/config.schema.json'), 'utf-8');
    schema = JSON.parse(content);
    expect(schema.$schema).toContain('json-schema.org');
    expect(schema.type).toBe('object');
  });

  it('has plugins section with safe defaults', async () => {
    const { readFile } = await import('node:fs/promises');
    const { resolve } = await import('node:path');
    const content = await readFile(resolve('config/config.schema.json'), 'utf-8');
    schema = JSON.parse(content);

    const plugins = schema.properties.plugins.properties;
    expect(plugins.enabled.default).toBe(false);
    expect(plugins.signatureRequired.default).toBe(true);
  });

  it('has captcha.provider restricted to enum', async () => {
    const { readFile } = await import('node:fs/promises');
    const { resolve } = await import('node:path');
    const content = await readFile(resolve('config/config.schema.json'), 'utf-8');
    schema = JSON.parse(content);

    const captcha = schema.properties.captcha.properties;
    expect(captcha.provider.enum).toEqual(['manual', '2captcha', 'none']);
  });

  it('has behavior section with clamped ranges', async () => {
    const { readFile } = await import('node:fs/promises');
    const { resolve } = await import('node:path');
    const content = await readFile(resolve('config/config.schema.json'), 'utf-8');
    schema = JSON.parse(content);

    const mouse = schema.properties.behavior.properties.mouse.properties;
    expect(mouse.steps.minimum).toBe(1);
    expect(mouse.steps.maximum).toBe(500);
    expect(mouse.jitterPx.maximum).toBe(20);
  });

  it('has workflows and plugins.instances extensible namespaces', async () => {
    const { readFile } = await import('node:fs/promises');
    const { resolve } = await import('node:path');
    const content = await readFile(resolve('config/config.schema.json'), 'utf-8');
    schema = JSON.parse(content);
    ...
    expect(schema.properties.plugins.properties.instances).toBeDefined();
  });

  it('has observability with otlp exporter option', async () => {
    const { readFile } = await import('node:fs/promises');
    const { resolve } = await import('node:path');
    const content = await readFile(resolve('config/config.schema.json'), 'utf-8');
    schema = JSON.parse(content);

    const exporter = schema.properties.observability.properties.metrics.properties.exporter;
    expect(exporter.enum).toContain('otlp');
  });
});
