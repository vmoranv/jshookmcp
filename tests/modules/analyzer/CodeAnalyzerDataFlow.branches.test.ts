...
vi.mock('@services/prompts/taint', () => ({ generateTaintAnalysisPrompt: promptState.generateTaintAnalysisPrompt }));
vi.mock('@modules/analyzer/SecurityCodeAnalyzer', () => ({ checkSanitizer: sanitizerState.checkSanitizer }));
...
describe('CodeAnalyzerDataFlow additional branch coverage', () => {
  ...
  describe('source detection network calls', () => {
    ...
    it('detects get post request axios', async () => {
      const r = await analyzeDataFlowWithTaint('const a=http.get("/"); const b=http.post("/"); const c=http.request("/"); const d=client.axios("/");');
  ...
  describe('source detection DOM query methods', () => {
    ...
    it('detects getElementsByClassName', async () => {
      const r = await analyzeDataFlowWithTaint('const els = document.getElementsByClassName("cls");');
      expect(r.sources.some(s => s.type === 'user_input')).toBe(true);
    });
  ...
  describe('LLM enhanced taint analysis', () => {
    ...
    it('calls LLM and adds unique paths', async () => {
      const llm = { chat: vi.fn().mockResolvedValue({ content: JSON.stringify({ taintPaths: [{ source: { type: 'network', location: { file: 'current', line: 99 } }, sink: { type: 'eval', location: { file: 'current', line: 100 } }, path: [] }] }) }) };
      const r = await analyzeDataFlowWithTaint('const s = location.href;\ndocument.body.innerHTML = s;', llm as any);
      expect(llm.chat).toHaveBeenCalled();
      expect(r.taintPaths.some(p => p.source.location.line === 99)).toBe(true);
    });
    it('skips duplicate LLM paths', async () => {
      const llm = { chat: vi.fn().mockResolvedValue({ content: JSON.stringify({ taintPaths: [{ source: { type: 'user_input', location: { file: 'current', line: 1 } }, sink: { type: 'xss', location: { file: 'current', line: 2 } }, path: [] }] }) }) };
      const r = await analyzeDataFlowWithTaint('const s = location.href;\ndocument.body.innerHTML = s;', llm as any);
    ...
    it('handles LLM response without taintPaths', async () => {
      const llm = { chat: vi.fn().mockResolvedValue({ content: JSON.stringify({ analysis: 'no paths' }) }) };
      const r = await analyzeDataFlowWithTaint('const s = location.href;\ndocument.body.innerHTML = s;', llm as any);
      expect(r.taintPaths.length).toBeGreaterThan(0);
    });
    ...
    it('handles LLM path without source or sink', async () => {
      const llm = { chat: vi.fn().mockResolvedValue({ content: JSON.stringify({ taintPaths: [{ source: null, sink: null, path: [] }, { source: { type: 'network', location: { file: 'current', line: 50 } } }] }) }) };
      const r = await analyzeDataFlowWithTaint('const s = location.href;\ndocument.body.innerHTML = s;', llm as any);
      expect(r.taintPaths.every(p => p.source && p.sink)).toBe(true);
    });
    it('uses empty array for missing LLM path', async () => {
      const llm = { chat: vi.fn().mockResolvedValue({ content: JSON.stringify({ taintPaths: [{ source: { type: 'network', location: { file: 'current', line: 88 } }, sink: { type: 'eval', location: { file: 'current', line: 89 } } }] }) }) };
      const r = await analyzeDataFlowWithTaint('const s = location.href;\ndocument.body.innerHTML = s;', llm as any);
