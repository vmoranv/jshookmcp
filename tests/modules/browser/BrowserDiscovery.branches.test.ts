...
describe('BrowserDiscovery additional branch coverage', () => {
  ...
  describe('parseWindowsResult', () => {
    ...
    it('returns empty for empty stdout', () => { expect(parse(discovery, '', '*')).toEqual([]); });
    it('returns empty for null stdout', () => { expect(parse(discovery, 'null', '*')).toEqual([]); });
    it('returns empty for whitespace', () => { expect(parse(discovery, '   \n  ', '*')).toEqual([]); });
    ...
    it('parses array', () => {
      const s = JSON.stringify([
        { ProcessId: 1, Handle: '0x1', Title: 'P - Google Chrome', ClassName: 'Chrome_WidgetWin_0' },
    ...
    it('identifies firefox by title', () => {
      const s = JSON.stringify({ ProcessId: 3, Handle: '0x3', Title: 'P - Mozilla Firefox', ClassName: 'MozillaWindowClass' });
      expect(parse(discovery, s, '*')[0].type).toBe('firefox');
    });
    it('falls back to class name matching', () => {
      const s = JSON.stringify({ ProcessId: 4, Handle: '0x4', Title: 'Unknown', ClassName: 'Edge_WidgetWin_1' });
      expect(parse(discovery, s, '*')[0].type).toBe('edge');
    });
    it('returns unknown for unmatched', () => {
      const s = JSON.stringify({ ProcessId: 5, Handle: '0x5', Title: 'Notepad', ClassName: 'NotepadWin' });
    ...
    it('returns empty for invalid JSON', () => { expect(parse(discovery, 'not json', '*')).toEqual([]); });
    it('matches wildcard class name pattern', () => {
      const s = JSON.stringify({ ProcessId: 7, Handle: '0x7', Title: 'Win', ClassName: 'Chrome_WidgetWin_99' });
  ...
  describe('parseProcessResult', () => {
    ...
    it('returns empty for empty stdout', () => { expect(parse(discovery, '', 'c')).toEqual([]); });
    it('returns empty for null stdout', () => { expect(parse(discovery, 'null', 'c')).toEqual([]); });
    ...
    it('identifies edge by msedge', () => {
      const s = JSON.stringify({ Id: 20, ProcessName: 'msedge', MainWindowHandle: 456, MainWindowTitle: 'E' });
      expect(parse(discovery, s, 'msedge')[0].type).toBe('edge');
    });
    it('identifies edge by edge', () => {
      const s = JSON.stringify({ Id: 21, ProcessName: 'Microsoft Edge', MainWindowHandle: 457, MainWindowTitle: 'E' });
      expect(parse(discovery, s, 'edge')[0].type).toBe('edge');
    });
    it('identifies firefox', () => {
      const s = JSON.stringify({ Id: 30, ProcessName: 'firefox', MainWindowHandle: 789, MainWindowTitle: 'F' });
      expect(parse(discovery, s, 'ff')[0].type).toBe('firefox');
    });
    it('returns unknown for unrecognized', () => {
      const s = JSON.stringify({ Id: 40, ProcessName: 'notepad', MainWindowHandle: 0, MainWindowTitle: '' });
    ...
    it('returns empty for invalid JSON', () => { expect(parse(discovery, '{broken!', 'x')).toEqual([]); });
  });

  describe('findByWindowClass', () => {
    it('calls powershell and returns results', async () => {
      execFileMock.mockResolvedValue({ stdout: JSON.stringify({ ProcessId: 100, Handle: '0x1', Title: 'T', ClassName: 'Chrome_WidgetWin_0' }) });
  ...
  describe('checkDebugPortFromCommandLine', () => {
    const check = (d: BrowserDiscovery, pid: number) => (d as any).checkDebugPortFromCommandLine.call(d, pid);

    it('returns debug port from cmdline', async () => {
      execFileMock.mockResolvedValue({ stdout: JSON.stringify({ CommandLine: 'chrome.exe --remote-debugging-port=9222', ParentProcessId: 1 }) });
      expect(await check(discovery, 100)).toBe(9222);
    });
    it('returns null when no debug port', async () => {
      execFileMock.mockResolvedValue({ stdout: JSON.stringify({ CommandLine: 'chrome.exe --no-sandbox', ParentProcessId: 1 }) });
    ...
    it('returns null for negative pid', async () => { expect(await check(discovery, -1)).toBeNull(); });
    it('returns null for NaN pid', async () => { expect(await check(discovery, NaN)).toBeNull(); });
    it('returns null for Infinity pid', async () => { expect(await check(discovery, Infinity)).toBeNull(); });
    it('returns null for zero pid', async () => { expect(await check(discovery, 0)).toBeNull(); });
    it('truncates fractional pid', async () => {
      execFileMock.mockResolvedValue({ stdout: JSON.stringify({ CommandLine: 'chrome.exe --remote-debugging-port=9333' }) });
      expect(await check(discovery, 100.9)).toBe(9333);
    });
  });

  describe('checkPort', () => {
    const checkPortFn = (d: BrowserDiscovery, pid: number, port: number) => (d as any).checkPort.call(d, pid, port);

    it('returns true when port found', async () => {
      execFileMock.mockResolvedValue({ stdout: JSON.stringify([{ LocalPort: 9222 }, { LocalPort: 443 }]) });
    ...
    it('returns false for invalid pid', async () => { expect(await checkPortFn(discovery, -1, 9222)).toBe(false); });
    it('returns false for invalid port', async () => { expect(await checkPortFn(discovery, 100, -1)).toBe(false); });
    it('returns false for NaN pid', async () => { expect(await checkPortFn(discovery, NaN, 9222)).toBe(false); });
    it('returns false for NaN port', async () => { expect(await checkPortFn(discovery, 100, NaN)).toBe(false); });
    it('returns false for Infinity pid', async () => { expect(await checkPortFn(discovery, Infinity, 9222)).toBe(false); });
    it('returns false for zero pid', async () => { expect(await checkPortFn(discovery, 0, 9222)).toBe(false); });
    it('returns false for zero port', async () => { expect(await checkPortFn(discovery, 100, 0)).toBe(false); });
  });

  describe('findBySignature private', () => {
    const findSig = (d: BrowserDiscovery, type: string, sig: BrowserSignature) => (d as any).findBySignature.call(d, type, sig);

    it('deduplicates by pid', async () => {
      vi.spyOn(discovery, 'findByProcessName' as any).mockResolvedValue([{ type: 'chrome', pid: 100, hwnd: '0x1' }]);
      vi.spyOn(discovery, 'findByWindowClass' as any).mockResolvedValue([{ type: 'chrome', pid: 100, hwnd: '0x2' }]);
      vi.spyOn(discovery, 'detectDebugPort').mockResolvedValue(null);
      const sig: BrowserSignature = { windowClasses: ['C_0'], processNames: ['chrome.exe'], debugPorts: [9222] };
      const r = await findSig(discovery, 'chrome', sig);
      expect(r).toHaveLength(1);
    });
    it('includes different pids', async () => {
      vi.spyOn(discovery, 'findByProcessName' as any).mockResolvedValue([{ type: 'chrome', pid: 100 }]);
      vi.spyOn(discovery, 'findByWindowClass' as any).mockResolvedValue([{ type: 'chrome', pid: 200 }]);
      vi.spyOn(discovery, 'detectDebugPort').mockResolvedValue(null);
      const sig: BrowserSignature = { windowClasses: ['C_0'], processNames: ['chrome.exe'], debugPorts: [9222] };
    ...
    it('attaches debug port', async () => {
      ...
      const sig: BrowserSignature = { windowClasses: [], processNames: ['chrome.exe'], debugPorts: [9222] };
      const r = await findSig(discovery, 'chrome', sig);
  ...
  describe('detectDebugPort', () => {
    ...
    it('probes ports in order', async () => {
      ...
      vi.spyOn(discovery as any, 'checkPort').mockResolvedValueOnce(false).mockResolvedValueOnce(true);
