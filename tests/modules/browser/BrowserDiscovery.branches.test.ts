...
function parseWindowsResult(
  mirror: BrowserDiscoveryMirror,
  stdout: string,
  pattern: string,
): BrowserInfo[] {
  return mirror.parseWindowsResult.call(mirror, stdout, pattern);
}
...
function parseProcessResult(
  ...
  return mirror.parseProcessResult.call(mirror, stdout, name);
}

function checkDebugPortFromCommandLine(
  mirror: BrowserDiscoveryMirror,
  pid: number,
): Promise<number | null> {
  return mirror.checkDebugPortFromCommandLine.call(mirror, pid);
}

function checkPort(mirror: BrowserDiscoveryMirror, pid: number, port: number): Promise<boolean> {
  return mirror.checkPort.call(mirror, pid, port);
}
...
function findBySignature(
  ...
  return mirror.findBySignature.call(mirror, type, signature);
}
...
describe('BrowserDiscovery additional branch coverage', () => {
  ...
  describe('parseWindowsResult', () => {
    const parse = parseWindowsResult;
  ...
  describe('parseProcessResult', () => {
    const parse = parseProcessResult;
  ...
  describe('checkDebugPortFromCommandLine', () => {
    const check = checkDebugPortFromCommandLine;
  ...
  describe('detectDebugPort', () => {
    ...
    it('probes ports in order', async () => {
      ...
      vi.spyOn(mirror, 'checkPort').mockResolvedValueOnce(false).mockResolvedValueOnce(true);
