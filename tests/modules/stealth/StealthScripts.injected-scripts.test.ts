...
describe("StealthScripts injected browser-side scripts", () => {
  beforeEach(() => { vi.restoreAllMocks(); resetInjectedPages(); });
  ...
  describe("mockBattery injected script", () => {
    ...
    it("wraps getBattery when it exists", async () => {
      ...
      (navigator as any).getBattery = () => Promise.resolve({ charging: false, chargingTime: 100, dischargingTime: 500, level: 0.5 });
      fn();
      const bat = await (navigator as any).getBattery();
      expect(bat).toBeDefined();
      if (orig) (navigator as any).getBattery = orig; else delete (navigator as any).getBattery;
    });
  });
  ...
  describe("mockNotifications injected script", () => {
    it("patches when Notification exists in window", async () => {
      ...
      try { expect(() => fn()).not.toThrow(); }
      finally {
        if (ow === undefined) delete (globalThis as any).window; else (globalThis as any).window = ow;
        if (oNotif === undefined) delete (globalThis as any).Notification; else (globalThis as any).Notification = oNotif;
      }
    });
    it("does nothing when Notification not in window", async () => {
      ...
      (globalThis as any).window = {};
      try { expect(() => fn()).not.toThrow(); }
      finally { if (ow === undefined) delete (globalThis as any).window; else (globalThis as any).window = ow; }
    });
  });
  ...
  describe("injectAll edge cases", () => {
    it("marks page and skips on re-call", async () => {
      const methods = ["hideWebDriver","mockChrome","mockPlugins","fixPermissions","mockCanvas","mockWebGL","fixLanguages","mockBattery","fixMediaDevices","mockNotifications"] as const;
    ...
    it("allows injection after WeakSet reset", async () => {
      ...
      ["mockChrome","mockPlugins","fixPermissions","mockCanvas","mockWebGL","fixLanguages","mockBattery","fixMediaDevices","mockNotifications"].forEach(m => vi.spyOn(StealthScripts, m as any).mockResolvedValue(undefined));
      const page = createPageMock();
      await StealthScripts.injectAll(page);
  ...
  describe("simple checks", () => {
    it("fixPermissions is a valid function", async () => { const p = createPageMock(); await StealthScripts.fixPermissions(p); expect(typeof getInjectedFn(p).fn).toBe("function"); });
    it("mockCanvas is a valid function", async () => { const p = createPageMock(); await StealthScripts.mockCanvas(p); expect(typeof getInjectedFn(p).fn).toBe("function"); });
    it("mockWebGL is a valid function", async () => { const p = createPageMock(); await StealthScripts.mockWebGL(p); expect(typeof getInjectedFn(p).fn).toBe("function"); });
    it("fixMediaDevices does not throw", async () => { const p = createPageMock(); await StealthScripts.fixMediaDevices(p); expect(() => getInjectedFn(p).fn()).not.toThrow(); });
