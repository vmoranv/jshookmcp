...
vi.mock('@modules/deobfuscator/ProApiClient', async () => {
  const actual = await import('./ProApiClient.ts');
  return {
    ...actual,
...
describe('Integration: Pro API & Anti-LLM', () => {
  ...
  it('deobfuscates code with Pro API token available', async () => {
    const code = 'var _0xabc = 123; function test() { return _0xabc; }';
  ...
  it('verifies LLM reconstruction quality', async () => {
    const original = 'var _0xabc123 = 123;';
    const llmOutput = 'var _0xdef456 = 123;'; // LLM changed variable names
