...
import { detectPrelude, carvePrelude, PreludeFunction, PreludeCarverResult } from '@modules/deobfuscator/PreludeCarver';
...
describe('PreludeCarver', () => {
  ...
  it('detectPrelude detects integrity/anti-debug patterns', () => {
    ...
    const result = detectPrelude(code);
    const integrityOrAntiDebug = result.find((p) => p.category === 'integrity' || p.category === 'anti-debug');
